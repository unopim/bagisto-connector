...
use Webkul\SunskyOnline\Enums\CacheType;
...
class AttributeController extends Controller
{
    ...
    {
        $sunskyAttributes = config('sunsky-attributes');

        $sunskyAttributes = $this->translate($sunskyAttributes);
        ...
        return view('sunsky-online::mappings.attribute', compact(
            'sunskyAttributes',
            'attributes',
            'standardAttributes',
    ...
    public function storeOrUpdate(): JsonResponse
    {
        try {
            ...
            return new JsonResponse([
                'message' => trans('sunsky-online::app.sunsky-plugin.sunsky-attributes.success-message'),
            ], 201);
        } catch (\Exception $e) {
    ...
    private function setFormatForMapping(array $data): array
    {
        ...
        foreach ($standardAttributesDefault as $key => $value) {
            if (empty($value)) {
                continue;
            }
    ...
    public function addAdditionalAttributes(Request $request)
    {
        ...
        $sunskyAttributes = config('sunsky-attributes');
        if (in_array($data['code'], array_column($sunskyAttributes, 'code'))) {
            return new JsonResponse([
    ...
    public function removeAdditionalAttributes(Request $request)
    {
        ...
        if ($standardAttributeObj) {
            if ($standardAttributeObj['mapped_value']) {
                ...
                foreach ($standardAttributeObj['mapped_value'] as $sunskyCode => $unopimCode) {
                    if ($sunskyCode == $request->code) {
                        continue;
                    }
                    $formatedData['mapped_value'][$sunskyCode] = $unopimCode;
                }
                foreach ($standardAttributeObj['fixed_value'] as $sunskyCode => $fixedValue) {
                    if ($sunskyCode == $request->code) {
                        continue;
                    }
                    $formatedData['fixed_value'][$sunskyCode] = $fixedValue;
                }
