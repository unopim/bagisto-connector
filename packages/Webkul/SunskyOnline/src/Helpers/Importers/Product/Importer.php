...
use Webkul\SunskyOnline\Helpers\CommonHelper;
...
class Importer extends AbstractImporter
{
    ...
    public const COMMON_IMAGE_GALLERY_ATTRIBUTE_CODE = 'gallery';
    ...
    public function saveProductsData(JobTrackBatchContract $batch): void
    {
        ...
        foreach ($batch->data as $rowData) {
            $this->prepareSuperAttributesAndOptions($rowData);
        ...
        $this->saveProductSuperAttributes($superAttributes);
    }

    /**
     * Prepare super attributes and options
     */
    public function prepareSuperAttributesAndOptions(array $product)
    {
        $modelLabel = $product['modelLabel'] ?? '';

        $attribute = $this->prepareAttributeForDB($modelLabel);

        $modelList = $product['modelList'] ?? [];

        $this->prepareOptionsForDB($modelList, $attribute);
    }

    /**
     * Prepare attribute for db
     */
    public function prepareAttributeForDB(string $modelLabel): Attribute
    {
        $attributeCode = CommonHelper::generateCode($modelLabel);
     ...
     */
    public function prepareOptionsForDB(array $modelList, Attribute $attribute)
    {
        $requestData = [];

        foreach ($modelList as $model) {
            $code = $model['value'];
            $option = $this->attributeOptionRepository->where('code', $code)->first();
            if (! $option) {
                $requestData['options'][] = [
                    'isNew' => 'true',
                    'code'  => $model['value'],
        ...
        if ($requestData) {
            $this->attributeRepository->update($requestData, $attribute->id);
        }
    }
    ...
    public function prepareConfigurableProducts(array $product, array &$configurableProducts, array &$superAttributes): void
    {
        ...
        if ($product['itemNo'] !== $product['groupItemNo'] && isset($product['modelList']) && $product['itemNo'] === $product['modelList'][0]['key']) {
            $product['product_number'] = $product['itemNo'];
            ...
            foreach ($productSuperAttributes as $attributeCode) {
                $attributeCode = strtolower(trim($attributeCode));
                $attributeId = $this->attributes->where('code', $attributeCode)->first()?->id;
    ...
    public function prepareProducts(array $product, array &$products, $productType = self::DEFAULT_PRODUCT_TYPE): void
    {
        ...
        if ($this->getFilter('in_detailed')) {
            ...
            if (isset($product['type']) && $product['type'] === 'configurable') {
                $searchItemNo = $product['product_number'];
            } else {
                $searchItemNo = $product['itemNo'];
            }
            $product = $this->openApiClient->getProductDetail($searchItemNo);
        }
        ...
        $superAttribute = $product['modelLabel'] ?? '';
        if ($superAttribute) {
            $superAttribute = CommonHelper::generateCode($superAttribute);

            foreach ($product['modelList'] as $index => $value) {
                if ($value['key'] === $product['itemNo']) {
                    $data['values']['common'][$superAttribute] = $value['value'];
                }
            }
        }

        // if ($this->getFilter('with_media')) {
    ...
    public function fetchMedia(array $product, array &$data): void
    {
        ...
        if (isset($product['type']) && $product['type'] === 'configurable') {
            $itemNo = $product['product_number'];
        }
        $media = $this->openApiClient->getProductMedia($itemNo);
        $images = [];

        if ($media) {
            $images = $this->getImagesFromZIP($media, $itemNo, self::COMMON_IMAGE_GALLERY_ATTRIBUTE_CODE);
        }

        if (! empty($images)) {
            $data['values']['common'][self::COMMON_IMAGE_GALLERY_ATTRIBUTE_CODE] = $images;
            unlink($media);
        }
    }
    ...
    {
        foreach ($superAttributes as $itemNo => $attributes) {
            if (isset($attributes['attribute_id'], $attributes['product_id'])) {
                $existingEntry = \DB::table('product_super_attributes')
