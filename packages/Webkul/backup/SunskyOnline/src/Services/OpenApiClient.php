...
class OpenApiClient
{
     ...
     */
    public function getProductDetail($itemNo)
    {
        return $this->callApi('product!detail.do', ['itemNo' => $itemNo]);
    }
    ...
    public function getProductMedia($itemNo, $size = null, $watermark = null)
    {
        ...
        $tempFilePath = sys_get_temp_dir().DIRECTORY_SEPARATOR.config('app.name').DIRECTORY_SEPARATOR."{$itemNo}.zip";
        if (! file_exists(dirname($tempFilePath))) {
            mkdir(dirname($tempFilePath), 0777, true);
        }

        return $this->downloadFile('product!getImages.do', $params, $tempFilePath);
    }
    ...
    protected function callApi(string $endpoint, array $parameters, $attempt = 1)
    {
        ...
        if ($response->successful()) {
            ...
            if ($responseData['result'] === 'error') {
                if ($attempt < 20) {  // Limit retries to 20 attempts
